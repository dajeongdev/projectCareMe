	...
	$(function() {
		$("#petSpeciesLevel1")
				.on(
						"change",
						function() {
							var ancestor = $(this).find("option:selected")
									.data("num");
							if (!ancestor) {
								$("#petSpeciesLevel2 option").remove();
								return false;
							}
							var url = "doctorWriteForm/pet_species_idx?level=2&ancestor="
									+ ancestor;
							$
									.ajax({
										type : "get",
									...
									.done(
											function(items) {
												$("#petSpeciesLevel2 option")
														.remove();
												if (items.length > 0) {
													for (item in items) {
														var s = items[item];
														var option = "<option value=" + s.pet_species_idx + ">"
																+ s.pet_species_name
																+ "</option>"
														$("#petSpeciesLevel2")
																.append(option);
		...
		form.onsubmit = function(e) {
			...
			$
					.ajax({
						...
						data : formData,
						success : function() {
							location.href = "/careMe/view/doctorBoardView/doctorBoard?currentPage=1";
						}
					})
		}
	})
			...
			<form name="addWrite" enctype="multipart/form-data">
					...
					<main role="main" class="col-lg-12 bg-white rounded shadow-sm">
						...
						<div class="row" style="width: 100%;">
							<div class="col-md-6  mb-3">
								<label for="petSpecies1">대분류</label> <select
									class="form-control" id="petSpeciesLevel1">
									<option>==선택==</option>
							...
							<div class="col-md-6  mb-3">
								<label for="petSpecies2">소분류</label> <select
									class="form-control" id="petSpeciesLevel2"
									name="pet_species_idx" required>
								</select>
							</div>
						</div>
						...
						<input type="text" class="form-control" id="tag" name="tag"
							placeholder="태그를 입력해보세요." style="margin-bottom: 0;">
						<div id="tag-list">
						</div>
						<p></p>
				...
				<div class="row">
					...
					<div class="col-md-12" align="center">
						...
						<input type="button" class="btn btn-dark btn-sm" value="목록으로"
							OnClick="location.href='doctorBoard?currentPage=1'">
					</div>

				</div>

			</form>
			<input type="file" class="form-control mb-3 d-none" id="files"
				name="image" placeholder="" max="5" multiple>
		</div>
	</div>
</body>
</html>